...
# Matches: password = "actual_value", SECRET_KEY: 'actual_value', apiToken="actual_value"
# Excludes env var references (process.env.*, import.meta.env.*, $ENV{...}), placeholder values,
# and common test fixtures (xxxxx, 00000, etc.)
if echo "$CONTENT" | grep -qiE '(password|secret|token|api_key|apikey|api_secret|private_key)[[:space:]]*[=:][[:space:]]*["\x27][^"\x27]{12,}["\x27]' && \
   ! echo "$CONTENT" | grep -qiE '(password|secret|token|api_key|apikey|api_secret|private_key)[[:space:]]*[=:][[:space:]]*["\x27]?(process\.env|import\.meta\.env|getenv|\$\{|ENV\[|env\(|Deno\.env|Bun\.env|config\.|loadEnv)'; then
