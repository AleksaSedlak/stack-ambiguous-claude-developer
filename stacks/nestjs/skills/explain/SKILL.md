...
- Edge cases and gotchas (null/undefined, empty arrays, race conditions)
- Dependencies and side effects (network calls, DB writes, file I/O, event emissions)
- Types and generics — what do the type parameters actually constrain?
- Server vs client boundaries (in Next.js: `use server` / `use client`), isomorphic vs runtime-specific code
...
- What would change if the type signature changed?
