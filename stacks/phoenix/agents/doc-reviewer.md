...
1. Run `git diff --name-only` via Bash to find changed documentation files (`.md`, `.txt`, `@doc`, `@moduledoc`, `@spec`, inline comments)
...
- **Function signatures**: read the actual function and verify parameter names, types in `@spec`, return types, and defaults match the `@doc`. Grep for the function name if needed.
