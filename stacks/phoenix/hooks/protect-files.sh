...
case "$FILE_PATH" in
  ...
  config/runtime.exs|*/config/runtime.exs)
    echo "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"deny\",\"permissionDecisionReason\":\"Cannot edit config/runtime.exs — this file contains production secrets and env var config.\"}}"
    exit 2
    ;;
esac
