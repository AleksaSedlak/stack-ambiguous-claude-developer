...
- **Git**: `git push origin main/master/staging/production`, `git push --force` (allows `--force-with-lease`), bare `git push` on protected branches
...
Auto-formats Elixir files (`.ex`, `.exs`) after Claude edits them. Finds the nearest `mix.exs` to determine the project root and runs `mix format` if `.formatter.exs` is present.
