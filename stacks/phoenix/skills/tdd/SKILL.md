...
- **Function signature** (e.g., `@spec deactivate_user(integer()) :: {:ok, User.t()} | {:error, atom()}`) → use the signature to derive behaviors directly.
...
2. The test must:
   - Describe the behavior in its name: `test "returns 0 for empty cart"`
