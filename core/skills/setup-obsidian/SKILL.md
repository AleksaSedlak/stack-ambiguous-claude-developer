...
- Use the last component of the current directory path (e.g. `/Users/me/Dev/my-next-app` → `my-next-app`)
...
Where `<encoded-project-path>` is the absolute project path with `/` replaced by `-` (e.g. `/Users/me/Dev/my-next-app` becomes `-Users-me-Dev-my-next-app`).
