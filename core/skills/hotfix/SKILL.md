...
- If `$ARGUMENTS` is an issue tracker ticket (e.g., `PROJ-123`): fetch it from the Jira MCP server if configured. If no Jira MCP is available, ask the user to paste the ticket description.
...
- Present to the user:
  ...
  >
  > **Suggested body**: what broke, what caused it, what this fixes
