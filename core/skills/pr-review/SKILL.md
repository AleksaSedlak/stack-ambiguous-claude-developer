...
2. **If security-sensitive code changed** — auth, input handling, DB queries, token/session code, cryptography, CORS/CSP configuration, dependency additions: delegate to `@security-reviewer`
3. **If performance-sensitive code changed** — endpoints, DB queries, loops over collections, caching, connection pools, rendering, bundle size, streams: delegate to `@performance-reviewer`
4. **If runtime/async-sensitive code changed** — async/await, Promises, streams, event emitters, workers, timers, lifecycle (startup/shutdown): delegate to `@runtime-reviewer` (if available for this stack)
5. **If documentation changed** — .md files, significant doc-comment changes, API specs, README: delegate to `@doc-reviewer`
